...
/*!50001 VIEW `album_songs` AS select `song`.`musicID` AS `songID`,`song`.`album_ID` AS `albumID`,`song`.`artistID` AS `artistID`,`song`.`Title` AS `Title`,`song`.`Explicit` AS `Explicit`,`song`.`Extension` AS `Extension`,`song`.`price` AS `song_price`,`album`.`album_name` AS `Album`,`album`.`release date` AS `Released`,`album`.`price` AS `album_price`,`users`.`username` AS `Artist` from ((`song` join `album` on((`song`.`album_ID` = `album`.`albumID`))) join `users` on((`song`.`artistID` = `users`.`user_id`))) */;
...
-- Dump completed on 2017-10-14 15:36:05
